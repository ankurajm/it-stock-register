...
const { generatePassword } = require('../utils/initials');
...
router.post('/add', requireAuth, requireAdmin, require('express-rate-limit')({ windowMs: 60 * 1000, max: 30, handler: (req, res) => { req.flash('error', 'Too many requests.'); res.redirect('/employees'); } }), async (req, res) => {
    try {
        const { emp_id, name, department, designation, joining_date, status, class_teacher, subject_teacher } = req.body;
        const joiningDateVal = joining_date || null;
        const initials = emp_id ? emp_id.trim().toUpperCase() : '';
        ...
        if (existingUser) {
            await run(`UPDATE users SET name=?, department=?, designation=?, joining_date=?, emp_status=?, class_teacher=?, subject_teacher=?, initials=? WHERE username=?`,
                [name, department, designation, joiningDateVal, status || 'active', class_teacher || '', subject_teacher || '', initials, emp_id]);
            req.flash('success', 'Employee ' + name + ' updated successfully');
        } else {
            const password = emp_id === 'TEST' ? 'test1234' : generatePassword();
            ...
            await run(`INSERT INTO users (username, password, initials, role, name, department, designation, email, phone, joining_date, emp_status, class_teacher, subject_teacher) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [emp_id, hashed, initials, 'user', name, department, designation, '', '', joiningDateVal, status || 'active', class_teacher || '', subject_teacher || '']);
            req.flash('success', `Employee ${name} added. Username: ${emp_id}, Password: ${password}, Initial: ${initials}`);
        }
...
router.post('/edit/:id', requireAuth, requireAdmin, require('express-rate-limit')({ windowMs: 60 * 1000, max: 30, handler: (req, res) => { req.flash('error', 'Too many requests.'); res.redirect('/employees'); } }), async (req, res) => {
    try {
        ...
        const joiningDateVal = joining_date || null;
        const initials = emp_id ? emp_id.trim().toUpperCase() : '';
        await run(`UPDATE users SET username=?, name=?, department=?, designation=?, joining_date=?, emp_status=?, class_teacher=?, subject_teacher=?, initials=? WHERE id=?`,
            [emp_id, name, department, designation, joiningDateVal, status, class_teacher || '', subject_teacher || '', initials, req.params.id]);
