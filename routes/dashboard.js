...
router.get('/', requireAuth, async (req, res) => {
    try {
        ...
        const totalEmployees = await get(`SELECT COUNT(*) as count FROM users WHERE name != ''`);
        ...
        const recentAllocations = await all(`SELECT a.*, i.asset_tag, u.name as emp_name FROM allocations a LEFT JOIN items i ON a.item_id = i.id LEFT JOIN users u ON a.employee_id = u.id ORDER BY a.allocated_date DESC LIMIT 5`);
